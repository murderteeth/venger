...
const hailing_prompt = (0, utils_1.template) `
...
router.post('/', async function (req, res, next) {
    ...
    const response = await (0, ai_1.one_shot)(hailing_prompt({ userPrompt }), .7);
    console.log('/hail prompt', response.data.usage);
    const hail = (0, ai_1.top_choice)(response);
    res.status(200).send({ ...JSON.parse(hail) });
});
